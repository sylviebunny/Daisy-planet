...
const (
      ...
      ES_URL = "http://35.235.121.229:9200"

      BUCKET_NAME = "zzx-post-images"
)
...
func handlerSearch(w http.ResponseWriter, r *http.Request) {
    ...
    if val := r.URL.Query().Get("range"); val != "" { 
      ...
      //return post data read from ElasticSearch
...
func createIndexIfNotExist() { //put data to es
      client, err := elastic.NewClient(elastic.SetURL(ES_URL), elastic.SetSniff(false)) //sniff: check if normal
      if err != nil {
            panic(err)
      }
...
func saveToES(post *Post, id string) error {
  ...
  // implement GeoDistanceQuery logic
  ...
  //search by using location range
